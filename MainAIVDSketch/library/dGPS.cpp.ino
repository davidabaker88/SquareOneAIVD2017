...
float angleToDes; //For slope method to build case statements
float slopeNumber; 
...
boolean dGPS::Compare() {
  ...
  for (int i = 0; i < min(strlen(recCheck), strlen(comCheck)); i++) {
    if (recCheck[i] == comCheck[i])
    {
      ...
      flag = false;  // if characters in checksum do not match
    }
  }
...
void dGPS::getflag(char *str) {
  ...
  while (Serial.available() == 0) {
    ; // If nothing available in serial buffer, wait and do nothing.
  }
  int index = 0;
    ...
    if (byteGPS == -1) {           // See if the port is empty yet
      ...
      if (byteGPS == 13) {         // If the received byte is = to 13, end of transmission
        ...
        if (bien == 6) {           // If yes, continue and process the data
          ...
          if (*hemi == 'S') {
            fLat = fLat * -1; // Setting the Latitude.
          }
          delay(5);
          ...
          if (*hemi == 'W') {
            fLon = fLon * -1; // Setting the Longitude.
          }
          delay(5);
...
float dGPS::angleToPoint() {
  angleToDes = atan((desLat - fLat ) / (desLon - fLon))*180/PI;
  if (desLat > fLat && desLon > fLon){
    angleToDes = 90 - angleToDes;
  }
  else if (desLat < fLat && desLon > fLon){
    angleToDes = 90 - angleToDes;
  }
  else if (desLat < fLat && desLon < fLon){
    angleToDes = -90 - angleToDes;
