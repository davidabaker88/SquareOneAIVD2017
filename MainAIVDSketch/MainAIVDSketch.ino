    ...
    else if (currentTask == five)
    {
        ...
        switch (t5Stage)
        {
            case 0: // Drive until a gap
                ...
                if (!!IRDistance(IR_FRONT_RIGHT_PIN))
                {
                    startDist = CountDistance(motorCount);
                    t5Stage++;
                }
            break;
            case 1: // Measure gap
                ...
                if (!IRDistance(IR_FRONT_RIGHT_PIN))
                {
                    if (CountDistance(motorCount) >= REQUIRED_SPACE + startDist) 
                    {
            ...
            case 2: // Stop when back sensor is at end of the gap
                ...
                setSpeed();
                steering(0);
                if (!!IRDistance(IR_BACK_RIGHT_PIN))
                {
                    Current = 0;
                    setBrake();
                }
